...
async function serversLoad(html: string): Promise<{ servers: Servers[]; title: string }> {
  try {
    ...
    console.error("Error in serversLoad:", error);
...
async function PRORCPhandler(prorcp: string): Promise<string | null> {
  try {
    ...
    console.error("Error in PRORCPhandler:", error);
    return null;
  }
}
...
async function rcpGrabber(html: string): Promise<RCPResponse | null> {
  try {
    ...
    console.error("Error in rcpGrabber:", error);
    return null;
  }
}

async function tmdbScrape(tmdbId: string, type: "movie" | "tv", season?: number, episode?: number): Promise<APIResponse[]> {
  try {
    ...
    const url = (type === "movie")
      ...
      : `https://vidsrc.xyz/embed/${type}?tmdb=${tmdbId}&season=${season}&episode=${episode}`;
    ...
    const rcpFetchPromises = servers.map(element => fetch(`${BASEDOM}/rcp/${element.dataHash}`));
    ...
    console.error("Error in tmdbScrape:", error);
    return [];
  }
}
